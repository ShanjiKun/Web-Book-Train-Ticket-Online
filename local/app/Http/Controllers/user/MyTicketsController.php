...
class MyTicketsController extends Controller
{
    public function getView(){
    	...
    	$ticketsPaid = DB::SELECT('SELECT * FROM ticket_cart tc inner join(SELECT bill_id FROM BILL WHERE user_id ='.$userID.' AND transaction_id IS NOT NULL) b on b.bill_id = tc.bill_id');
    	...
    	return Utils::reateResponse( 0, '{"waitSeat": "'.$waitSeatsJSON.'", "ticketsPaid": "'.json_encode($ticketsPaid).'", "ticketsUnpaid": "'.json_encode($ticketsUnpaid).'"}');
    }
}
